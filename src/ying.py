...
#     W_v = 1/(np.abs(gauker_v)*np.abs(dt0_v)+sharpness)
    ...
    W_v = 1/(np.abs(gv)*np.abs(dt0_v)+sharpness)
...
def tsmooth(img, lamda=0.01, sigma=3.0, sharpness=0.001):
    I = cv2.normalize(img.astype(fDtype), None, 0.0, 1.0, cv2.NORM_MINMAX)
    x = np.copy(I)
...
def rgb2gm(I):
    if (I.shape[2] == 3):
        I = cv2.normalize(I.astype(fDtype), None, 0.0, 1.0, cv2.NORM_MINMAX)
...
def Ying_2017_CAIP(img, mu=0.5, a=-0.3293, b=1.1258):
    ...
    t_our = cv2.resize(
        tsmooth(
            cv2.resize(t_b, (0,0), None, 0.5, 0.5, cv2.INTER_CUBIC),
#             scipy.misc.imresize(t_b, 0.5, interp='bicubic', mode='F'), 
            lamda, 
            sigma), 
